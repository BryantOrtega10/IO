...
#Busca el minimo coste en toda la matriz de costos, devuelve las coordenadas de ese coste
def buscarMinimoCoste(tabla):
    aux = None
    pos = (0,0)
    for i in range(len(tabla)):
        for j in range(len(tabla[i])):
            if((aux is None or aux > tabla[i][j].costo) and tabla[i][j].estado):
                aux = tabla[i][j].costo
                pos = (i,j)
    return pos
...
while not demandas_ofertas_0(demanda, oferta):
    #Toma las coordenadas del minimoCoste y las asigna a las variables x, y
    x, y = buscarMinimoCoste(tabla)
